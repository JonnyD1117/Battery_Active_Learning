...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        self.logger.record('train/SOC', env.cur_soc)
        self.logger.record('train/Full SOC', env.full_soc)
        self.logger.record('train/Reward', env.re)
        self.logger.record('train/Concentration', env.C_se)
        self.logger.record('train/Concentration 2', env.C_se1)
        self.logger.record('train/Sensitivity', env.epsi_sp.item())
        self.logger.record('train/Sen', env.dCse_dEpsi)
...
if __name__ == '__main__':
    ...
    model = DDPG('MlpPolicy', env, action_noise=action_noise, verbose=1, tensorboard_log=log_dir, learning_rate=.1)
