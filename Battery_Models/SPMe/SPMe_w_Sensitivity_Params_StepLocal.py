...
class SingleParticleModelElectrolyte_w_Sensitivity(SPMe_Baseline_Parameters):
    def __init__(self, custom_params=None, timestep=1, sim_time=3600):
        ...
        if custom_params is not None:
            pass
            # self.param = self.import_custom_parameters(new_param_dict)
    ...
    def import_custom_parameters(self, param_dict):
        ...
        # stoi_x0 = 0.0069     # Stoich. Coef. for Negative Electrode
        # stoi_x100 = 0.6760
        #
        # stoi_y0 = 0.8228     # Stoich. Coef for Positive Electrode
        # stoi_y100 = 0.442
    ...
    def step(self, states=None, I_input=None, state_of_charge=None, full_sim=False):
        ...
        if states is None:
            ...
            xn_old = np.array([[(stoi_n * self.param['cs_max_n']) / (self.param['rfa_n'] * 10395 * (self.param['Ds_n'] ** 2))], [0], [0]])  # stoi_n100 should be changed if the initial soc is not equal to 50 %
            xp_old = np.array([[(stoi_p * self.param['cs_max_p']) / (self.param['rfa_p'] * 10395 * (self.param['Ds_p'] ** 2))], [0], [0]])  # initial positive electrode ion concentration
            xe_old = np.array([[0], [0]])
        ...
        vel = (-I * (0.5 * self.param['Lp'] + 0.5 * self.param['Ln']) / (self.param['Ar_n'] * self.param['kappa_eff']) + (-I * self.param['Lsep']) / (self.param['Ar_n'] * self.param['kappa_eff_sep']) + (2 * self.param['R'] * self.param['T'] * (1 - self.param['t_plus']) * (1 + 1.2383) * np.log((1000 + yep_new[0].item()) / (1000 + yep_new[1].item()))) / self.param['F'])  # yep(1, k) = positive boundary;
        R_e = -I * (0.5 * self.param['Lp'] + 0.5 * self.param['Ln']) / (self.param['Ar_n'] * self.param['kappa_eff']) + (-I * self.param['Lsep']) / (self.param['Ar_n'] * self.param['kappa_eff_sep'])
        V_con = (2 * self.param['R'] * self.param['T'] * (1 - self.param['t_plus']) * (1 + 1.2383) * np.log((1000 + yep_new[0].item()) / (1000 + yep_new[1].item()))) / self.param['F']
        phi_n = 0
        phi_p = phi_n + vel

        # Compute "Exchange Current Density" per Electrode (Pos & Neg)
        i_0n = self.param['kn'] * self.param['F'] * (self.param['cen'] * yn_new * (self.param['cs_max_n'] - yn_new)) ** .5
        i_0p = self.param['kp'] * self.param['F'] * (self.param['cep'] * yp_new * (self.param['cs_max_p'] - yp_new)) ** .5

        # Kappa (pos & Neg)
        k_n = Jn / (2 * self.param['as_n'] * i_0n)
        k_p = Jp / (2 * self.param['as_p'] * i_0p)

        # Compute Electrode "Overpotentials"
        eta_n = (self.param['R']*self.param['T']*np.log(k_n + (k_n**2 + 1)**0.5))/(self.param['F']*0.5)
        eta_p = (self.param['R']*self.param['T']*np.log(k_p + (k_p**2 + 1)**0.5))/(self.param['F']*0.5)

        # Record Stoich Ratio (SOC can be computed from this)
        theta_n = yn_new / self.param['cs_max_n']
        theta_p = yp_new / self.param['cs_max_p']
        ...
        V_term = (U_p - U_n) + (eta_p - eta_n) + vel - self.param['Rf'] * I / (self.param['Ar_n'] * self.param['Ln'] * self.param['as_n'])  # terminal voltage
        R_film = -self.param['Rf'] * I / (self.param['Ar_n'] * self.param['Ln'] * self.param['as_n'])
